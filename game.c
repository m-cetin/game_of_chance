...
// A Game of Chance
// cleaned code, copied from "The Art of Exploitation" by Jon Erickson
...
int main() {
    ...
    while (choice != 7) {
        ...
        printf("7 - Quit\n");
        printf("[Name: %s]\n", player.name);
        printf("[You have %d credits] -> ", player.credits);
        scanf("%d", &choice);
...
// This function displays the current high score and the name of
// the person who set that high score.
void show_highscore() {
...
// This function prompts the player to pick a number between 1 and 20.
// If the player picks the winning number, they win 100 credits.
// It returns -1 if the player has less than 10 credits.
int pick_a_number() {
    int pick, winning_number;
    printf("\n####### Pick a Number ######\n");
    printf("This game costs 10 credits to play. Simply pick a number between 1 and 20, and if you pick the winning number, you will win the jackpot of 100 credits!\n\n");
...
int find_the_ace() {
    ...
    while (invalid_choice) { // Loop until valid choice is made.
        ...
        if (choice_two == 'c') { // Change pick.
            ...
            while (i == pick || cards[i] == 'Q') { // Loop until the other card is found, and then swap pick.
                i++;
            }
